...
from accounts.models import UserGroups, GroupPermissons
...
def check_permission(user, method, permission_to):
    ...
    for group in groups:
        ...
        permissions = GroupPermissons.objects.values('permission_id').filter(group_id=group['group_id']).all()
...
class EmployeePermission(permissions.BasePermission):
    message = 'User does not have permission to manage employee'
...
class GroupPermissionPermission(permissions.BasePermission):
    message = 'User does not have permission to manage permissions'
...
class TaskPermission(permissions.BasePermission):
    message = 'User does not have permission to manage tasks'
